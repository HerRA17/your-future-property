...
export default function UpdateListing() {
    ...
    const handleImageSubmit = () => {
        ...
        if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
            ...
            Promise.all(promises).then((urls) => {
                ...
                setImageUploadError('Image upload failed (max 4 mb per Image)');
                setUploading(false);
            })
        } else {
    ...
    const storeImage = async (file) => {
        return new Promise((resolve, reject) => {
            ...
            uploadTask.on(
                ...
                (error) => {
                    console.error('Upload error: ', error, 'for file: ', file.name )
                    reject(error);
                },
                ()=> {
                    getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
                        ...
                        console.error('Error geting download URL: ', error, 'for file: ', file.name )
                        reject(error);
                    });
                }
            )
  ...
  return (
    ...
    <h1 className='text-3xl font-semibold text-center my-7'>Updatete Listing</h1>
    <form className='flex flex-col gap-3 sm:flex-row' onSubmit={handleSubmit}>
        <div className='flex flex-col gap-4 flex-1' id='column-left-top'>
            ...
            id='name' maxLength='62' minLength='10' required onChange={handleChange}
            value={formData.name}/>
            ...
            <div className='flex items-center gap-2' id='prices'>
                <div className='flex items-center gap-2'>
                    <input type='number' id='regularPrice' min='50' max='100 000 000' required
                    className='p-3 border border-gray-300 rounded-lg' 
                ...
                {formData.offer && (<div className='flex items-center gap-2'>
                    <input type='number' id='discountPrice' min='0' max='1 000 000' required
                    className='p-3 border border-gray-300 rounded-lg' 
